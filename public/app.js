...
async function api(url,opt={}){const r=await fetch(url,opt);if(r.status===401){window.location.href="/";throw new Error("Login required")}const j=await r.json();if(!r.ok)throw new Error(j.error||"Request failed");return j}
