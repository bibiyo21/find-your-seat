...
// and the "Add Table" endpoint (with the decimal-seats bug fixed).
const assert = require("node:assert/strict");

async function api(baseUrl, path, opt = {}) {
...
async function runApiSuite(baseUrl) {
  // Create event
  let res = await api(baseUrl, "/api/events", {
    method: "POST",
  ...
  try {
    ...
    res = await api(baseUrl, "/api/events");
    ...
    res = await api(baseUrl, `/api/events/${eventId}`);
    ...
    res = await api(baseUrl, `/api/events/${eventId}/tables`, {
      method: "POST",
      headers: {"Content-Type": "application/json"},
    ...
    res = await api(baseUrl, `/api/events/${eventId}/tables/${table1Id}`, {
      method: "PUT",
      headers: {"Content-Type": "application/json"},
    ...
    res = await api(baseUrl, `/api/events/${eventId}/layout`, {
      method: "PUT",
      headers: {"Content-Type": "application/json"},
    ...
    res = await fetch(`${baseUrl}/api/events/${eventId}/import`, {method: "POST", body: form});
    const importBody = await res.json();
    assert.equal(res.status, 200, "excel import should succeed");
    ...
    // Excel export
    res = await fetch(`${baseUrl}/api/events/${eventId}/export/excel`);
    assert.equal(res.status, 200, "excel export should succeed");
    assert.match(res.headers.get("content-type") || "", /spreadsheetml/);

    // Guest search + reassign table
    res = await api(baseUrl, `/api/events/${eventId}/search?q=Alice`);
    assert.equal(res.status, 200);
    assert.equal(res.body.length, 1);
    assert.equal(typeof res.body[0].id, "string", "guest id must be a string");
    const aliceId = res.body[0].id;

    res = await api(baseUrl, `/api/events/${eventId}/guests/${aliceId}`, {
      method: "PUT",
      headers: {"Content-Type": "application/json"},
    ...
    res = await api(baseUrl, `/api/events/${eventId}/tables/${table3.id}`, {method: "DELETE"});
    ...
    // QR code
    res = await api(baseUrl, `/api/events/${eventId}/qr`);
    assert.equal(res.status, 200);
    assert.match(res.body.qrCode, /^data:image\//);

    // Print page
    res = await fetch(`${baseUrl}/events/${eventId}/print`);
    assert.equal(res.status, 200, "print page should be served");
  } finally {
    // Clean up so the test never leaves residue behind, even against a real DB.
    await api(baseUrl, `/api/events/${eventId}`, {method: "DELETE"});
  }
