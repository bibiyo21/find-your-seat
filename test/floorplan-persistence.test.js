...
async function api(baseUrl, p, opt = {}) {
  const r = await fetch(baseUrl + p, opt);
...
test("floor plan drag interactions persist to the database", async () => {
  ...
  try {
    ...
    await api(baseUrl, `/api/events/${eventId}/tables`, {
      method: "POST", headers: {"Content-Type": "application/json"},
    ...
    await fetch(`${baseUrl}/api/events/${eventId}/import`, {method: "POST", body: form});
    ...
    // --- 1. Moving a table persists its new location ---
    res = await api(baseUrl, `/api/events/${eventId}/tables/${table1.id}`, {
      method: "PUT", headers: {"Content-Type": "application/json"},
      body: JSON.stringify({x: 542, y: 217})
    });
    assert.equal(res.status, 200, "moving a table should succeed");

    res = await api(baseUrl, `/api/events/${eventId}`); // fresh fetch, like reopening the page
    ...
    res = await api(baseUrl, `/api/events/${eventId}/guests/${guest.id}`, {
      method: "PUT", headers: {"Content-Type": "application/json"},
