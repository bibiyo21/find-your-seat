...
app.get("/api/events", async (req,res) => {
  try {
    ...
    for (const e of events) {
      const table_count = await database.collection("tables").countDocuments({event_id: e._id});
      const guest_count = await database.collection("guests").countDocuments({event_id: e._id});
      const tables = await database.collection("tables").find({event_id: e._id}).toArray();
      const seat_count = tables.reduce((a,t) => a+t.seats, 0);
...
app.post("/api/events", async (req,res) => {
  try {
    ...
    const result = await database.collection("events").insertOne({name, created_at: new Date().toISOString()});
...
app.listen(PORT, () => console.log(`Wedding Seating Planner: http://localhost:${PORT} (MongoDB)`));
